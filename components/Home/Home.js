...
const Home = (props) => {
  ...
  // const urlLink = useSelector((state) => state.urlList);
  ...
  // const newLink = props.props?.Oldlink

  // console.log(newId);

  // // const oldId = link?.id;

  // const oldId = props.props?.linkId;
  // console.log(oldId);
  useEffect(() => {
    if (showUrl) {
      createId();
          ...
          {urlItems.map((item) => (
            <div key={item.linkId} className="linkCont">
              <div className="linkDiv">
               <a style={{ marginBottom: "15px", color: "grey"}} rel="noreferrer" target="_blank" href={`${item.url}`}>
                  {item?.url?.length > 27 ? (
                ...
                <a href={`/${item.linkId}`} style={{ color: "rgba(43, 170, 164, 0.911)"}} rel="noreferrer" target="_blank" >shutly.vercel.app/{item.linkId}</a>
                <CopyToClipboard
